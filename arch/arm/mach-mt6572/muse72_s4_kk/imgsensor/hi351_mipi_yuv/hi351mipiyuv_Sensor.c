...
static bool capture_flag = 0;
static int hi351_antibanding = HI351_50HZ;
static int hi351_video_fps;
static bool hi351_InitSet = KAL_FALSE;
//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-04-08] HI351 Reduce the Shutter Lag
static bool HI351_Flash_OnOff = 0;
static kal_uint16 HI351_flash_mode = FLASH_MODE_FORCE_OFF;
//[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-04-08] HI351 Reduce the Shutter Lag
...
void HI351_set_Sleep_off()
{
	...
	if(hi351_InitSet == KAL_TRUE)
	{
		hi351_InitSet = KAL_FALSE;
		return;
	}
...
void HI351PreviewSetting(void)
{
	...
	if(HI351CurrentScenarioId == MSDK_SCENARIO_ID_VIDEO_PREVIEW)
	{
		if (hi351_video_fps == 15)
		{
 ...
 *
 *************************************************************************/
extern int soc_antibanding; //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-15] HI351 Manual Flicker
...
{
	if(g_lge_camera==1) //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-22] SBP Camera Sendor Check
	{
		*sensorID =0xffffffff;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	volatile signed char i = 0;
	int retry = 3; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-05] Read SensorID retry 3.
	...
	//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-05-05] Read SensorID retry 3.
	do
	{
	...
	//[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-05-05] Read SensorID retry 3.
...
UINT32 HI351Open(void)
{
	...
	mdelay(5);
	hi351_antibanding = soc_antibanding; //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-15] HI351 Manual Flicker
...
BOOL HI351_set_param_wb(UINT16 para)
{
	...
	switch (para)
	{
		...
		case AWB_MODE_AUTO:
			...
			//AWB target RG angle
			HI351_write_cmos_sensor(0xaa, 0x1e);//aInRgTgtOfs_a00_n00
			HI351_write_cmos_sensor(0xab, 0x14);//aInRgTgtOfs_a01_n00
			HI351_write_cmos_sensor(0xac, 0x0a);//aInRgTgtOfs_a02_n00
			HI351_write_cmos_sensor(0xad, 0x05);//aInRgTgtOfs_a03_n00
			...
			HI351_write_cmos_sensor(0xb1, 0x8a);//aInRgTgtOfs_a07_n00

			//AWB target BG angle
			HI351_write_cmos_sensor(0xb2, 0x9e);//aInBgTgtOfs_a00_n00
			HI351_write_cmos_sensor(0xb3, 0x94);//aInBgTgtOfs_a01_n00
			HI351_write_cmos_sensor(0xb4, 0x8a);//aInBgTgtOfs_a02_n00
			HI351_write_cmos_sensor(0xb5, 0x85);//aInBgTgtOfs_a03_n00
			...
			HI351_write_cmos_sensor(0xb9, 0x0a);//aInBgTgtOfs_a07_n00

			HI351_write_cmos_sensor(0x03, 0xc6);
			HI351_write_cmos_sensor(0x18, 0x50); // InRgainMin
			HI351_write_cmos_sensor(0x19, 0xf0); // InRgainMax
			HI351_write_cmos_sensor(0x1a, 0x40); // InBgainMin
			HI351_write_cmos_sensor(0x1b, 0x9f); // InBgainMax

			HI351_write_cmos_sensor(0xb9, 0x68); // OutRgainMin
			HI351_write_cmos_sensor(0xba, 0x88); // OutRgainMax
			HI351_write_cmos_sensor(0xbb, 0x4c); // OutBgainMin
			HI351_write_cmos_sensor(0xbc, 0x6c); // OutBgainMax
...
BOOL HI351_set_param_scene(UINT16 para)
{
	...
	switch (para)
	{
		...
		// [LGE_UPDATE_S][youmi.jun@lge.com][2014.04.08] Modify SCENE_MODE
		case SCENE_MODE_OFF:
		default:
			//		HI351_TRACE("[HI351]:SCENE_MODE_OFF: =%d\n", para);
			hi351_scene_normal_mode();
			break;
		// [LGE_UPDATE_E][youmi.jun@lge.com][2014.04.08] Modify SCENE_MODE
	}
		...
		MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	HI351_Flash_OnOff = 0; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-08] HI351 Reduce the Shutter Lag
		...
		MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pSensorInfo->CaptureDelayFrame = HI351_Flash_OnOff ? 2 : 0; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-08] HI351 Flash Tuning - Timing
	...
	switch (ScenarioId)
	{
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			...
			pSensorInfo->SensorGrabStartY = 0; //[LGE_UPDATE][youmi.jun@lge.com] Remove offset to resolve issue about preview, picture mismatch
			break;
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
...
static void HI351GetExifInfo(UINT32 pSensorExifInfoStruct)
{
	...
	Info->FNumber = 28;         // [LGE_UPDATE][youmi.jun@lge.com][2014.03.39] Add FNumber
	...
	Info->FocalLength = 270;    // [LGE_UPDATE][youmi.jun@lge.com][2014.03.39] Add FocalLength
...
//[LGE_UPDATE_S] [yonghwan.lym@lge.com] [2014-03-18] HI351 Auto Flash
...
//[LGE_UPDATE_E] [yonghwan.lym@lge.com] [2014-03-18] HI351 Auto Flash
