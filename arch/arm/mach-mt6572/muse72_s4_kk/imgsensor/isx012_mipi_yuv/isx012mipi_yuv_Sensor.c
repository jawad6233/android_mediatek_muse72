...
//[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-04] Flash Tuning - read data,sequence
...
//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-04-04] Flash Tuning - read data,sequence
...
//[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-04-04] Flash Tuning - read data,sequence
static struct
{
	//kal_uint8   Banding;
	kal_bool	  NightMode;
	...
	//[LGE_UPDATE_S] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
	...
	short int ERSC_now;
	kal_uint32 AE_scl;
	//[LGE_UPDATE_E] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
	kal_uint32 video_fps; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
...
static void ISX012MIPIinitalvariable(void)
{
	...
	ISX012MIPISensor.exposure_value = 0;  //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
	ISX012MIPISensor.flash_mode = FLASH_MODE_FORCE_OFF;  //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
	ISX012MIPISensor.userAskAeLock=KAL_FALSE;
    ISX012MIPISensor.userAskAwbLock=KAL_FALSE;
	ISX012MIPISensor.video_fps = 30; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
...
// [LGE_UPDATE_S][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
static kal_uint16 ISX012MIPIGetRealIso(void)
{
	...
	// [LGE_UPDATE][youmi.jun@lge.com][2014.04.25] Add ISO boundary condition because ISO40 is displayed ISO8000 in gallery
	if (iso_idx <= 2)
		iso_idx = 3;  // ISO_50

	return (ISX012MIPISensor.isoSpeed == AE_ISO_AUTO) ? iso_tbl[iso_idx] : ISX012MIPISensor.isoSpeed;
}
// [LGE_UPDATE_E][youmi.jun@lge.com][2014.02.10]
...
void ISX012MIPIGetExifInfo(UINT32 exifAddr)
{
	...
	pExifInfo->FNumber = 27;                         // [LGE_UPDATE][youmi.jun@lge.com][2014.03.39] Modify FNumber
	pExifInfo->AEISOSpeed = ISX012MIPIGetRealIso();  // [LGE_UPDATE][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
	pExifInfo->AWBMode = ISX012MIPISensor.awbMode;
	pExifInfo->CapExposureTime = 0;
	pExifInfo->FlashLightTimeus = 0;
	pExifInfo->RealISOValue = pExifInfo->AEISOSpeed; // [LGE_UPDATE][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
	pExifInfo->FocalLength = 343;                    // [LGE_UPDATE][youmi.jun@lge.com][2014.04.01] Update FocalLength
...
{
	//[LGE_UPDATE_S] [yonghwan.lym@lge.com] [2014-03-03] HI351 Bring Up
	if(g_lge_camera==0) //[LGE_UPDATE][yonghwan.lym@lge.com][2014-04-22] SBP Camera Sendor Check
	{
		*sensorID =0xffffffff;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	//[LGE_UPDATE_E] [yonghwan.lym@lge.com] [2014-03-03] HI351 Bring Up
...
*
*************************************************************************/
//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-05-14] Add timeout for prevent endless loop.
...
//[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-05-14] Add timeout for prevent endless loop.
/*************************************************************************
* FUNCTION
...
{
	//[LGE_UPDATE_S] [youmi.jun@lge.com] [2014-04-22] Improve AF in low light condition
...
static void ISX012_FOCUS_Set_AF_Window(UINT32 zone_addr)
{
	...
	if (((x0==124) && (y0==84) && (x1==195) && (y1==155)) ||  // 4:3 default
	    ...
	    ((x0==160) && (y0==120) && (x1==160) && (y1==120)))   //[LGE_UPDATE_S][youmi.jun@lge.com][2014-04-25] Add AF reset condition
	{
	...
	ISX012MIPI_write_cmos_sensor1_16(0x6A56, v); // V size //[LGE_UPDATE][youmi.jun@lge.com][2014-04-19] Bug fix
	//
	ISX012MIPISENSORDB("[ISX012_FOCUS_Set_AF_Window] Set focusing with 3A window data\n");
	ISX012MIPI_write_cmos_sensor1(0x6A80,0x00);  //[LGE_UPDATE][youmi.jun@lge.com][2014-04-19] Improve touch af
...
static void ISX012_FOCUS_Set_AE_Window(UINT32 zone_addr)
{
	...
	//[LGE_UPDATE_S][youmi.jun@lge.com][2014-04-25] Add AE window reset condition
	if ((x0==160) && (y0==120) && (x1==160) && (y1==120))
	{
		ISX012MIPI_write_cmos_sensor1(0x0188, 0x0);
		return;
	}
	//[LGE_UPDATE_E][youmi.jun@lge.com][2014-04-25] Add AE window reset condition
	...
	//[LGE_UPDATE_S][youmi.jun@lge.com][2014-04-19] Bug fix about touch ae
	...
	//[LGE_UPDATE_E][youmi.jun@lge.com][2014-04-19] Bug fix about touch ae
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	//[LGE_UPDATE_S] [youmi.jun@lge.com] [2014-04-01] Add camcorder mode
	...
	{
		//[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
		if(ISX012MIPISensor.video_fps == 15)
		{
			// fire mode line
			ISX012MIPI_write_cmos_sensor1(0x0356,0x42);// SHTCTRLTIME1_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x0357,0x3C);// AGCGAIN1_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x0358,0x42);// SHTCTRLTIME2_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x0359,0x3C);// AGCGAIN2_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x035A,0x29);// SHTCTRLTIME3_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x035B,0x3C);// AGCGAIN3_TYPE9 :

			ISX012MIPI_write_cmos_sensor1(0x0308,0x02); /* AELINE_MONI_SN1_2 */
			ISX012MIPI_write_cmos_sensor1_16(0x018E,0x041C); /* VADJ_SENS_1_2 */
		}
		else //24-30 fps
		{
			ISX012MIPI_write_cmos_sensor1(0x0308,0x11); /* AELINE_MONI_SN1_2 */
			ISX012MIPI_write_cmos_sensor1_16(0x018E,0x0012); /* VADJ_SENS_1_2 */
			// fire mode line
			ISX012MIPI_write_cmos_sensor1(0x0356,0x42);// SHTCTRLTIME1_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x0357,0x3C);// AGCGAIN1_TYPE9 :
			ISX012MIPI_write_cmos_sensor1(0x0358,0x42);// SHTCTRLTIME2_TYPE9 :
		...
		//[LGE_UPDATE_E][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
		...
		//[LGE_UPDATE_S] [youmi.jun@lge.com] [2014-04-16] Add CAF setting to reduce sensitivity of AF
		...
		//[LGE_UPDATE_E] [youmi.jun@lge.com] [2014-04-16] Add CAF setting to reduce sensitivity of AF
	...
	//[LGE_UPDATE_E] [youmi.jun@lge.com] [2014-04-01] Add camcorder mode
...
UINT32 ISX012MIPIGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
	...
	//[LGE_UPDATE_S][youmi.jun@lge.com] Remove offset to resolve issue about preview, picture mismatch
	...
	//[LGE_UPDATE_E][youmi.jun@lge.com] Remove offset to resolve issue about preview, picture mismatch
...
UINT32 ISX012MIPIGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId,MSDK_SENSOR_INFO_STRUCT *pSensorInfo,MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	//[LGE_UPDATE_S] [youmi.jun@lge.com] [2014-04-18] Increase CaptureDelayFrame for stabilize AWB (trade-off: increase shutter-lag)
	pSensorInfo->CaptureDelayFrame = ISX012MIPISensor.Flash_OnOff ? 2 : 0; //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-04-07] Reduce the Camera Shutter Lag
	//pSensorInfo->CaptureDelayFrame = 1;
	//[LGE_UPDATE_E] [youmi.jun@lge.com] [2014-04-18] Increase CaptureDelayFrame for stabilize AWB (trade-off: increase shutter-lag)
...
UINT32 ISX012MIPIYUVSetVideoMode(UINT16 u2FrameRate)
{
	...
	ISX012MIPISensor.video_fps = u2FrameRate; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
	ISX012MIPI_write_cmos_sensor1_16(0x665A, 0x00D0); //[LGE_UPDATE][youmi.jun@lge.com][2014-04-01] To reduce vcm tic noise (near:0x01F4, far:0x00C8)
...
static void ISX012MIPI_FlashAE()
{
	...
	//[LGE_UPDATE_S] [yonghwan.lym@lge.com] [2014-04-19] Flash Tuning - Read AE value
	if(ISX012MIPISensor.Flash_AE_Start == 2)
	{
		// Read data for Flash AE
		ISX012MIPISensor.ERSC_now = ISX012MIPIYUV_read_cmos_sensor1_16(0x01CC);
	...
	//[LGE_UPDATE_E] [yonghwan.lym@lge.com] [2014-04-19] Flash Tuning - Read AE value
...
UINT32 ISX012MIPIFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId,UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_AE_FLASHLIGHT_INFO:
			ISX012MIPI_FlashAE(); //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
		...
		case SENSOR_FEATURE_SET_FLASHLIGHT:
			ISX012MIPISensor.flash_mode = *pFeatureData16; //[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-03] Add auto flash
		...
		//[LGE_UPDATE_S][youmi.jun@lge.com][2014.03.24] Get FOV from sensor
		case SENSOR_FEATURE_GET_SENSOR_VIEWANGLE:
			...
			break;
		//[LGE_UPDATE_E][youmi.jun@lge.com][2014.03.24] Get FOV from sensor
