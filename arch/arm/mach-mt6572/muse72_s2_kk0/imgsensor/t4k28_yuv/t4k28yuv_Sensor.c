...
#include "t4k28yuv_Sensor_lgit.h"
#include "t4k28yuv_Sensor_cowell.h"

static int gCameraId = 1; // 0:LGIT. 1:COWELL
...
struct
{
  ...
  kal_uint8   isoSpeed;	// [LGE_UPDATE][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
...
static void T4K28YUV_Sensor_Init(int lines)
{
    ...
    gCameraId = GetCameraId();
    if (gCameraId) // COWELL
    {
        SENSORDB("lln:: T4K28YUV_Sensor_Init: COWELL\n");
        len = sizeof(T4K28_Init_Reg_Cowell)/sizeof(T4K28_Init_Reg_Cowell[0]);
        T4K28_table_write_cmos_sensor(T4K28_Init_Reg_Cowell,len,T4K28_WRITE_ID);
    }
    else // LGIT
    {
        SENSORDB("lln:: T4K28YUV_Sensor_Init: LGIT\n");
        len = sizeof(T4K28_Init_Reg_Lgit)/sizeof(T4K28_Init_Reg_Lgit[0]);
        T4K28_table_write_cmos_sensor(T4K28_Init_Reg_Lgit,len,T4K28_WRITE_ID);
    }
...
UINT32 T4K28YUVOpen(void)
{
	...
	t4k28yuv_status.isoSpeed = AE_ISO_100;	// [LGE_UPDATE][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
	t4k28yuv_status.video_fps = 30; //[LGE_UPDATE][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if(CurrentScenarioId == MSDK_SCENARIO_ID_VIDEO_PREVIEW)
	{
		...
		if(t4k28yuv_status.video_fps == 15)
		{
			T4K28YUV_write_cmos_sensor(0x351B,0x10); //Fix 15fps
		}
		else
		{ //15-30 fps
													...
													MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    pSensorInfo->CaptureDelayFrame = 1;//[LGE_UPDATE] [yonghwan.lym@lge.com] [2014-03-01] Reduce the Shutter Lag
...
UINT32 T4K28YUVSetVideoMode(UINT16 u2FrameRate)
{
    ...
    //[LGE_UPDATE_S][yonghwan.lym@lge.com][2014-05-13] use "fix 15fps" at camcorder MMS size.
    t4k28yuv_status.video_fps = u2FrameRate;
#if 0
    if(u2FrameRate == 15) {
			T4K28YUV_write_cmos_sensor(0x351B,0x18);  //FAUTO/FCOUNT[2:0]/FCLSBON/EXPLIM[2:0] //Fix 15fps
...
void T4K28_set_iso(UINT16 para)
{
    ...
    spin_unlock(&T4K28_drv_lock);
    // [LGE_UPDATE_E][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
...
void T4K28GetExifInfo(UINT32 exifAddr)
{
    ...
    pExifInfo->AEISOSpeed = T4K28GetRealIso();       // [LGE_UPDATE][youmi.jun@lge.com][2014.02.10] Update real iso value in case of ISO_AUTO
    ...
    pExifInfo->FocalLength = 270;                    // [LGE_UPDATE][youmi.jun@lge.com][2014.03.39] Add FocalLength
}
